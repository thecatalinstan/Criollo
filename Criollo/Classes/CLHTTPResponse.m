...
@implementation CLHTTPResponse(Private)
...
- (void)sendHTTPStatus
{
	...
	NSDictionary* userInfo = @{CLRequestKey: self.HTTPRequest.FCGIRequest, CLDataKey: data == nil ? [NSData data] : data };
    [[CLApplication sharedApplication] writeDataToStdout:userInfo];
}
...
{
    return [[CLHTTPResponse alloc] initWithHTTPRequest:anHTTPRequest];
}
...
{
    NSDictionary* userInfo = @{CLRequestKey: self.HTTPRequest.FCGIRequest, CLDataKey: data == nil ? [NSData data] : data };
    [[CLApplication sharedApplication]  writeDataToStderr:userInfo];
}
...
- (void)finish
{
    ...
    [[CLApplication sharedApplication]  finishRequest:self.HTTPRequest.FCGIRequest];
}
